# Java Heap Memory Monitor - Old Generation Only
# This script monitors Java heap memory usage (Old Generation only) and sends alerts when thresholds are exceeded
...
function Send-Alert {
    ...
    $body = "${message}: Old Generation heap usage is at $usage% on host $($config.manualHostname)."
    ...
    try {
        ...
        Write-Log "$message alert sent: Old Generation heap usage is at $usage%."
...
Write-Host "Java Heap Memory Monitor (Old Generation Only) started"
    ...
    if ($jstatOutput) {
        ...
        if ($lines.Count -gt 1) {
            ...
            if ($heapData.Count -ge 5) {
                # Get Old Generation usage (index 3)
                $oldGenUsageString = $heapData[3]
                $oldGenUsage = 0.0

                if ([float]::TryParse($oldGenUsageString, [ref]$oldGenUsage)) {
                    Write-Log "Current Old Generation heap usage: $oldGenUsage%"

                    # Check if heap usage exceeds the threshold
                    if ($oldGenUsage -gt $config.threshold) {
                        if (-not $alertSent) {
                            Send-Alert -usage $oldGenUsage -message "Warning"
                            $alertSent = $true  # Set flag to true after sending the alert
                        }
                    } else {
                        if ($alertSent) {
                            # System returned to normal, send normal alert once
                            Send-Alert -usage $oldGenUsage -message "Normal"
                            $alertSent = $false  # Reset flag when memory usage is back to normal
                        }
                    }
                } else {
                    Write-Log "Old Generation heap usage value is not numeric: '$oldGenUsageString'"
                }
            } else {
                Write-Log "Unexpected output format from jstat."
            }
        } else {
