    ...
    if ($jstatOutput) {
        ...
        if ($lines.Count -gt 1) {
            ...
            if ($heapData.Count -ge 5) {
                $heapUsageString = $heapData[3]  # O column index
                $heapUsage = 0.0                 # Initialize the variable

                if ([float]::TryParse($heapUsageString, [ref]$heapUsage)) {
                    Write-Log "Current heap usage: $heapUsage%"
                    ...
                    Write-Log "Heap usage value is not numeric: '$heapUsageString'"
                }
            } else {
